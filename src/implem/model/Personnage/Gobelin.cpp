...
void Gobelin::drop() {
    ...
    if (drop_item == 0){
        auto* Epee_Gobelin = new Equipement("Epee_Gobelin", OT_Equipement, "Attaque:15 Defense:10", PT_Guerrier, false, 15, 10, 0, 1);
        pushSac(Epee_Gobelin);
    }
    if (drop_item == 1){
        auto* Lance_Gobelin = new Equipement("Lance_Gobelin",OT_Equipement,"Attaque:10 Defense:5 Sante:15",PT_Amazone,false,10, 5, 15, 1);
        pushSac(Lance_Gobelin);
    }
    if (drop_item == 2){
        auto* Masse_Gobelin = new Equipement("Masse_Gobelin",OT_Equipement,"Attaque:5 Defense:10 Sante:15",PT_Moine,false,5, 5, 15, 1);
        pushSac(Masse_Gobelin);
    }
    if (drop_item == 3){
        auto* Baguette_Gobelin = new Equipement("Baguette_Gobelin",OT_Equipement,"Attaque:15 Defense:5 Sante:5",PT_Sorcier,false,15, 5, 5, 1);
